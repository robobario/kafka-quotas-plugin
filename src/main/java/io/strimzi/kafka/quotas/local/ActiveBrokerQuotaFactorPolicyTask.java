...
public class ActiveBrokerQuotaFactorPolicyTask implements QuotaFactorPolicyTask {
        ...
        try {
            ...
            for (String brokerId : activeBrokers) {
                ...
                for (Volume volume : brokerSnapshot.getVolumes()) {
                    quotaFactor = updateQuotaFactor(quotaFactor, quotaFactorPolicy.quotaFactor(volume));
                }
                if (isPause(quotaFactor)) {
                    break;
                }
            }
            double updatedQuotaFactor = quotaFactor;
    ...
    private void notifyNewQuota(double updatedQuotaFactor) {
    ...
    private boolean isPause(double newFactor) {
        return newFactor >= -EPSILON && newFactor <= EPSILON;
    }

    private double updateQuotaFactor(double quotaRemaining, double missingDataQuotaFactor) {
        return Math.min(quotaRemaining, missingDataQuotaFactor);
    }
    ...
    private void updateMetricsCache() {
        for (VolumeUsageMetrics metricsUpdate : volumeUsageMetricsSupplier.get()) {
            ...
            if (lastKnown != null && metricsUpdate.getSnapshotAt().isAfter(lastKnown.getSnapshotAt())) {
                ...
                log.warn("ignoring volumeMetrics as they are too old: {} currently using snapshot generated at {}", metricsUpdate.getSnapshotAt(), lastKnown.getSnapshotAt());
            }
        }
    }
